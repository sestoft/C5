...
public class HashedArrayList<T> : ArrayBase<T>, IList<T>, SCG.IList<T>
{
    ...
    private readonly HashSet<SCG.KeyValuePair<T, int>> itemIndex;
    #endregion
    #region Events
    ...
    {
        itemIndex = new HashSet<SCG.KeyValuePair<T, int>>(new KeyValuePairEqualityComparer<T, int>(itemequalityComparer));
    }
    ...
    void RemoveAll(Func<T, bool> predicate)
    {
        ...
        int removed = 0;
        int i = offsetField, end = offsetField + size;
        SCG.KeyValuePair<T, int> p = new SCG.KeyValuePair<T, int>();
        while (i < end)
        {
            T item;
            //pass by a stretch of nodes
    ...
    #region SC.ICollection Members

    bool SC.ICollection.IsSynchronized => false;

    [Obsolete]
    object SC.ICollection.SyncRoot => underlying != null ? ((SC.ICollection)underlying).SyncRoot : array;

    void SC.ICollection.CopyTo(Array arr, int index)
    {
    ...
    bool SC.IList.Contains(object o)
    {
        return Contains((T)o);
    }

    int SC.IList.IndexOf(object o)
    {
        return Math.Max(-1, IndexOf((T)o));
    }

    void SC.IList.Insert(int index, object o)
    {
        Insert(index, (T)o);
    }

    void SC.IList.Remove(object o)
    {
        Remove((T)o);
    }
