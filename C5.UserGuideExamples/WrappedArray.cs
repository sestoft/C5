...
//  dotnet build -p:StartupObject=C5.UserGuideExamples.WrappedArray
//  dotnet run
...
namespace C5.UserGuideExamples
{
    class WrappedArray
    {
        ...
        {
            var j = new WrappedArray<T>(arr).View(i, arr.Length - i).FindIndex(p);
            return j < 0 ? j : j + i;
        }
        ...
        {
            var j = new WrappedArray<T>(arr).View(i, n).IndexOf(x);
            return j < 0 ? -1 : j + i;
        }
        ...
        {
            var j = new WrappedArray<T>(arr).View(i, arr.Length - i).LastIndexOf(x);
            return j < 0 ? -1 : j + i;
        }
