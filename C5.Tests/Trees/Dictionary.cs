...
namespace C5.Tests.trees.RBDictionary
{
    ...
    public class Enumerators
    {
        ...
        public void KeysISorted()
        {
            ...
            Assert.IsTrue(keys.UniqueItems().All(s => s is "R" or "S" or "T"));
            Assert.IsTrue(keys.All(s => s == "R" || s == "S" || s == "T"));
            Assert.IsFalse(keys.Exists(s => s != "R" && s != "S" && s != "T"));
            Assert.IsTrue(keys.Find(s => s == "R", out string res));
            Assert.AreEqual("R", res);
            Assert.IsFalse(keys.Find(s => s == "Q", out res));
